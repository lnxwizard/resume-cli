...
fn main() {
    ...
    loop {
        ...
        if choice == options[0] {
            ...
            let contents =
                fs::read_to_string(SKILLS_FILE_PATH).expect("Couldn't find skills.toml file");

            // print skills
            print_skills(&contents);
        } else if choice == options[2] {
            let contents =
                fs::read_to_string(TOOLS_FILE_PATH).expect("Couldn't find tools.toml file.");

            // print tools
            print_tools(&contents);
        } else if choice == options[3] {
            let contents =
                fs::read_to_string(SOCIAL_FILE_PATH).expect("Couldn't find social.toml file.");
